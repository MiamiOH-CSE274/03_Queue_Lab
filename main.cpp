...
void testRemoveException(ArrayQueue<int>& testQueue){
	...
	catch (std::string s) { //if a string is thrown then it is caught here
		std::cout << "SUCCESS: Caught exception: " << s << std::endl;
		return;
	}
	catch (...) { //if anything other than a string is thrown then it is caught here
...
int main(){
	ArrayQueue<int> testQueue; //Fills in an int everywhere that 'T' is stored and saves it as the variable testQueue
	ArrayQueue<int>* queue2 = new ArrayQueue<int>();
	delete queue2;
